  ...
  methods: {
    ...
    async callAcctList() {
      try {
        ...
        if (this.acctBucketName.trim().length > 0) {
          body.bucketName = this.acctBucketName
        } else {
          delete body.bucketName
        }
        if (this.acctUsername.trim().length > 0) {
    ...
    rowCellEdit(idx, obj, type) {
      console.log('row index:>>', idx, '; obj:>>', obj)
      if (this.tabName === '0' && type === 'edit') {
        // 权限
      ...
      } else if (this.tabName === '1' && type === 'authManager') {
        // TODO 人员权限
      }
    },
    ...
    roleTransferResult(val) {
      ...
      this.formValidate(this, 'roleForm')
        .then(() => {
          ...
          me.proxy(me, me.apiList.roles, method, obj)
            .then(({ data }) => {
